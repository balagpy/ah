...
class SettingsController < ApplicationController
  ...
  def destroy
    ...
    @user.soft_delete
    sign_out
    redirect_to root_path, notice: "The account was deleted successfully."
  end
  ...
  def auth_unbind
    ...
    if current_user.legacy_omniauth_logined?
      redirect_to account_setting_path, alert: "Three-party account login without setting email and password, unbinding is not allowed, please set the password and modify the email."
      return
    end
  ...
  def update_password
    if @user.update_with_password(user_params)
      redirect_to new_session_path(:user), notice: "The password has been updated successfully, now you need to log in again"
    else
      render "password"
    end
  end
